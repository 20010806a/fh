...
raw_data = pd.read_csv(r'E:\四川盆地\四川迁移\CAV\CAV2.csv',encoding='GBK')
...
data_df_xunlian.columns=['CAV','zh']
data_xunlian=pd.DataFrame(X_train)
data_xunlian.columns=['Rrup','M','Vs30']
writer_xunlian=pd.ExcelWriter(r'E:\四川盆地\四川迁移\CAV\train1.xlsx')
...
data_df_ceshi.columns=['CAV','zh']
data_ceshi=pd.DataFrame(X_test)
data_ceshi.columns=['Rrup','M','Vs30']
writer_ceshi=pd.ExcelWriter(r'E:\四川盆地\四川迁移\CAV\test1.xlsx')
...
model_svr = SVR(kernel="rbf",gamma=0.279,C=4.216,epsilon=0.195) # Set the hyperparameters of SVR GMMs
model_names = ['SVR']
model_dic = [model_svr]
cv_score_list = []
pre_y_list = []
...
worksheet.write(0, 5, label="CAV")
worksheet.write(0, 6, label="Rrup")
new_pre_SVR_y = []
raw_data = pd.read_csv(r"E:\四川盆地\四川迁移\归4.8_406.csv",encoding='GBK') # input the information for prediction (R,M,Vs30)
...
data=[]
for i in range(len(y_test)):
     e_times_pred = math.exp(new_pre_SVR_y[i])
     worksheet.write(i+1,1, label=new_pre_SVR_y[i][0])
     worksheet.write(i+1,2,label=X_test1[i][0])
     worksheet.write(i+1,3,label=X_test1[i][1])
     worksheet.write(i+1,4,label=X_test1[i][2])
     worksheet.write(i + 1, 5, label=e_times_pred)
     worksheet.write(i + 1, 6, label=i + 1)
workbook.save('1fh.xls') # The prediction results given by SVR GMMs
print(X_test1[0][0])