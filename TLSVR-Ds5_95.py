...
raw_data = pd.read_csv(r'E:\四川盆地\四川迁移\D95\D952.csv',encoding='GBK')
...
data_df_xunlian.columns=['D90','zh']
data_xunlian=pd.DataFrame(X_train)
data_xunlian.columns=['ln_distance','M','ln_Vs30']
writer_xunlian=pd.ExcelWriter(r'E:\四川盆地\四川迁移\D95\train1.xlsx')
...
data_df_ceshi.columns=['D90','zh']
data_ceshi=pd.DataFrame(X_test)
data_ceshi.columns=['ln_distance','M','ln_Vs30']
writer_ceshi=pd.ExcelWriter(r'E:\四川盆地\四川迁移\D95\test1.xlsx')
...
model_svr = SVR(kernel="rbf",gamma=0.279,C=4.473,epsilon=0.124) # Set the hyperparameters of SVR GMMs
model_names = ['SVR']
model_dic = [model_svr]
cv_score_list = []
pre_y_list = []
