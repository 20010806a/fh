...
raw_data = pd.read_csv(r'E:\四川盆地\四川迁移\D75\D752.csv', encoding='GBK')
X = raw_data.iloc[:, [2, 3, 4]].values
y = raw_data.iloc[:, [0, 1]].values
...
data_df_xunlian.columns = ['FileName','DS5-75']
data_xunlian = pd.DataFrame(X_train)
data_xunlian.columns = ['R', 'M', 'Vs30']
writer_xunlian = pd.ExcelWriter(r'E:\四川盆地\四川迁移\D75\train1.xlsx')
data_df_xunlian.to_excel(writer_xunlian)
data_xunlian.to_excel(writer_xunlian, startcol=3)
writer_xunlian._save()

# Testing set
data_df_ceshi = pd.DataFrame(y_test)
data_df_ceshi.columns = ['FileName','DS5-75']
data_ceshi = pd.DataFrame(X_test)
data_ceshi.columns = ['R', 'M', 'Vs30']
writer_ceshi = pd.ExcelWriter(r'E:\四川盆地\四川迁移\D75\test1.xlsx')
...
raw_data = pd.read_csv(r'E:\四川盆地\四川迁移\D75\D752.csv', encoding='GBK')
X = raw_data.iloc[:, [2, 3, 4]].values
y = np.log(raw_data.iloc[:, 1].values)
...
model_svr = SVR(kernel="rbf", gamma=0.279,C=4.206,epsilon=0.166)  # Set the hyperparameters of SVR GMMs
model_names = ['SVR']
model_dic = [model_svr]
cv_score_list = []
pre_y_list = []
...
# 写入标题行
headers = ['Actual Value', 'SVR Value', 'R', 'M', 'Vs30']
for col_num in range(len(headers)):
    worksheet.write(0, col_num, label=headers[col_num])

# 写入数据
for i in range(len(y_train)):
    worksheet.write(i + 1, 0, label=y_train[i])
    worksheet.write(i + 1, 1, label=pre_y_list[0][i])
    worksheet.write(i + 1, 2, label=X_train[i][0])
    worksheet.write(i + 1, 3, label=X_train[i][1])
    worksheet.write(i + 1, 4, label=X_train[i][2])

# 保存Excel文件
workbook.save('1SVR_true_predict_training_set.xls')
...
workbook.save('1SVR_true_predict_test_set.xls')
...
new_pre_SVR_y = []
raw_data = pd.read_csv(r'E:\四川盆地\四川迁移\归6.6_399.csv',encoding='GBK')  # input the information for prediction (R,M,Vs30)
...
workbook.save('3fh.xlsx')  # The prediction results given by SVR GMMs
print("Excel文件已保存。")
print(X_test1[0][0])