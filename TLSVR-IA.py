...
raw_data = pd.read_csv(r'E:\四川盆地\四川迁移\IA\IA2.csv',encoding='GBK')
...
data_df_xunlian.columns=['zh','IA']
data_xunlian=pd.DataFrame(X_train)
data_xunlian.columns=['ln_distance','M','ln_Vs30']
writer_xunlian=pd.ExcelWriter(r'E:\四川盆地\四川迁移\IA\train1.xlsx')
...
data_df_ceshi.columns=['zh','IA']
data_ceshi=pd.DataFrame(X_test)
data_ceshi.columns=['ln_distance','M','ln_Vs30']
writer_ceshi=pd.ExcelWriter(r'E:\四川盆地\四川迁移\IA\test1.xlsx')
...
model_svr = SVR(kernel="rbf",gamma=0.279,C=12.557,epsilon=0.379)  # Set the hyperparameters of SVR GMMs
model_names = ['SVR']
model_dic = [model_svr]
cv_score_list = []
pre_y_list = []
for model in model_dic:#交叉验证
...
df1 = pd.DataFrame(cv_score_list, index=model_names)#交叉验证的结果
...
print(type(y_test))
data=[]
#将存储在new_pre_SVR_y列表中的所有预测结果平铺到一个新的列表data中
...
workbook.save('3fh.xls') # The prediction results given by SVR GMMs
print(X_test1[0][0])
